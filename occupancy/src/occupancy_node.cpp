...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
        if (framesSinceLastPublished > 5)
        {
            ...
            // std::cout << occGrid->mapMetaData.width << ' ' << occGrid->mapMetaData.height << std::endl;
            // for (int i = 0; i < occGrid->mapMetaData.width * occGrid->mapMetaData.height; i++)
            // {
            //     std::cout << (int) occGrid->data[i] << " ";
            // }
            // std::cout << std::endl;
